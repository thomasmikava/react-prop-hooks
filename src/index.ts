...
type PropChangeAsArray<T> = <K extends number>(
  key: K
) => (fn: PropChangeFn<T, GetValueType<T, K> | undefined, K>) => Unsubscribe;

type PropChangeAsObject<T> = <K extends KeysOfUnion<T>>(
  key: K
) => (fn: PropChangeFn<T, GetValueType<T, K>, K>) => Unsubscribe;

interface SubscribeToFns<T> {
  change: (fn: ChangeFn<T>) => Unsubscribe;
  propChange: { a: NonNullable<T> } extends { a: readonly any[] }
    ? PropChangeAsArray<NonNullable<T>>
    : PropChangeAsObject<NonNullable<T>>;
  anyPropChange: (
